...
def imprimeInstrucciones():
    ...
    print 'Comando incorrecto. Utilizacion correcta:'
    print '\tpython probing.py <archivo> <x> <y> <dx> <dy> <prof_z>'
    print '\tpython probing.py <archivo> <x> <y>\n'
...
if not (len(sys.argv) in [4, 7]):
    ...
    try:
        ...
        dx = int(sys.argv[4])
        dy = int(sys.argv[5])
        prof_z = float(sys.argv[6])
    except ValueError:
        sys.exit('Error en la conversion de argumentos.')
else:
    try:
        length_x = int(sys.argv[2])
        length_y = int(sys.argv[3])
...
if os.path.isfile(filename):
    ...
    # Modificar el archivo original con el mapa
    print 'Modificando el archivo original...'
    modificarArchivo(filename, f, prof_z)

else:
    print 'El archivo indicado no es correcto.'    
